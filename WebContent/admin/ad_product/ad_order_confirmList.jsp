       ...
       if(confirm("해당주문을 승인하시겠습니까?")){
          if($('.season_check').is(':checked')){
             ...
             $("input[name=order]:checked").each(function(i){
            	 pdName.push($(this).val());
             })
             
             // 담은 체크박스 값들을 해당 경로로 보낸다.
             window.location = "${path}/orderConfirm.do?shopNo="+shopNo;
          }
          else{
    ...
    $("#chkDelete").click(function(){
       if(confirm("해당주문건을 환불처리하시겠습니까?")){
          if($('.season_check').is(':checked')){
             ...
             // 담은 체크박스 값들을 해당 경로로 보낸다.
             window.location = "${path}/saleRefundAction.do?shopNo="+shopNo;
          }
          else{
             alert('삭제할 주문을 선택하세요');
          }
               ...
               <form method=post name="cartForm">
                     ...
                     <tr>
                     	<td colspan="12" align="center">
                     		...
                     		<c:if test="${paging.startPage > 10}">
                     			<a href="${path}/orderConfirmList.do?pageNum=${paging.prev}">[이전]</a>
                     		</c:if>
                     		
                     		<!-- 페이지번호 처리  -->
                     		<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
                     			<a href="${path}/orderConfirmList.do?pageNum=${num}">${num}</a>
                     		</c:forEach>
                     		
                     		<!-- 다음 버튼 활성화 여부 -->
                     		<c:if test="${paging.endPage < paging.pageCount}">
                     			<a href="${path}/orderConfirmList.do?pageNum=${paging.next}">[다음]</a>
                     		</c:if>
                     	</td>
                     </tr>
                  </table>
                <!--  <input type="button" value="구매하기" class="btnBuy"  style="display:right; width:100%; text-align:center"> -->
                 ...
                 <div class="cart__mainbtns" align="center" >
			         <input type="button" id="chkDelete"  class="cartDelete" value="환불처리"> 
			      </div>
               </div>
            </div>
            <!--- 우측 메뉴 종료 --->
