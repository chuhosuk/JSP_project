    ...
    $("#chkBuy").click(function(){
       if(confirm("선택상품을 구매하시겠습니까?")){
          if($('.season_check').is(':checked')){
             ...
             window.location = "${path}/buyCartAction.do?cartNo="+cartNo;
          }
          else{
    ...
    $("#chkDelete").click(function(){
       if(confirm("선택상품을 삭제하시겠습니까?")){
          if($('.season_check').is(':checked')){
             ...
             // 담은 체크박스 값들을 해당 경로로 보낸다.
             window.location = "${path}/cartDeleteAction.do?cartNo="+cartNo;
          }
          else{
             alert('삭제할 상품을 체크해주세요');
          }
               ...
               <form method=post name="cartForm">
                  <table class="table table-bordered" id="cartList">
			            ...
			            <c:forEach var="dto" items="${list}">
	                   	<tr class="cart__list__detail">
	                   		<td><input type="checkbox" name="cart" class="season_check" value="${dto.cartNo}"></td>
	                    	<td>${dto.cartNo}</td>
                     	...
                     	<td colspan="6" align="center">
                     		...
                     		<c:if test="${paging.startPage > 10}">
                     			<a href="${path}/cartList.do?pageNum=${paging.prev}">[이전]</a>
                     		</c:if>
                     		
                     		<!-- 페이지번호 처리  -->
                     		<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
                     			<a href="${path}/cartList.do=${num}">${num}</a>
                     		</c:forEach>
                     		
                     		<!-- 다음 버튼 활성화 여부 -->
                     		<c:if test="${paging.endPage < paging.pageCount}">
                     			<a href="${path}/cartList.do=${paging.next}">[다음]</a>
                     		</c:if>
                     	</td>
                     </tr>
                  </table>
                <!--  <input type="button" value="구매하기" class="btnBuy"  style="display:right; width:100%; text-align:center"> -->
                 ...
                 <div class="cart__mainbtns" align="center" >
			         <input type="button" id="chkDelete"  class="cartDelete" value="상품삭제하기"> 
			         
			         <input type="button" id="chkBuy"  class="cartBuy" value="상품구매">
			      </div>
               </div>
            </div>
            <!--- 우측 메뉴 종료 --->
