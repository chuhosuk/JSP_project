    ...
    $("#chkDelete").click(function(){
       if(confirm("주문을 취소하시겠습니까?")){
          if($('.season_check').is(':checked')){
             ...
             // 체크된 박스 값을 배열에 담는다.
             $("input[name=myOrder]:checked").each(function(i){
            	 shopNo.push($(this).val());
             })
             
             // 담은 체크박스 값들을 해당 경로로 보낸다.
             window.location = "${path}/refund.do?shopNo="+shopNo;
          }
          else{
                  ...
                  <table class="table table-bordered" id="myOrder">
			            ...
			            <c:forEach var="dto" items="${list}">
	                   	<tr class="cart__list__detail">
	                   		<td><input type="checkbox" name="myOrder" class="season_check" value="${dto.shopNo}"></td>
	                    	<td>${dto.shopNo}</td>
	                    	<td>${dto.pdName}</td>
	                    	<td>${dto.pdPrice}원</td>
                     	...
                     	<td colspan="7" align="center">
                     		...
                     		<c:if test="${paging.startPage > 10}">
                     			<a href="${path}/myOrder.do?pageNum=${paging.prev}">[이전]</a>
                     		</c:if>
                     		
                     		<!-- 페이지번호 처리  -->
                     		<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
                     			<a href="${path}/myOrder.do?pageNum=${num}">${num}</a>
                     		</c:forEach>
                     		
                     		<!-- 다음 버튼 활성화 여부 -->
                     		<c:if test="${paging.endPage < paging.pageCount}">
                     			<a href="${path}/myOrder.do?pageNum=${paging.next}">[다음]</a>
                     		</c:if>
                     	</td>
                     </tr>
                  </table>
